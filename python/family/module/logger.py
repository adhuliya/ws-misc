...
logFileDir = ".itsoflife/local/logs/family-logs"
logFileName = "family.log"
logFileFullDirPath = "./family-logs" # default value
...
def createLogDir():
  ...
  if userHome:
    ...
    if not os.path.exists(logFileFullDirPath):
      print("family: Attempting to create {} dir.".format(osp.join(userHome, logFileDir)))
      p = subprocess.getstatusoutput("mkdir -p {}".format(logFileFullDirPath))
      if p[0] != 0:
        print("family: failed: Can't create log dir {}: {}".format(logFileFullDirPath, p[1]))
        return False
    print("family: Using log dir: {}".format(logFileFullDirPath))
    return True  # success
  else:
    print("family: failed: Can't create log dir {}: Can't find user home.".format(logFileDir))
    return False
...
def initLogger(logFile=None, logLevel=logLevel, logFormat=logFormat2, logMaxSize=logMaxSize,
  ...
  status = createLogDir()
  if not status:
    print("family: failed: Can't create logging dir; hence logging disabled.")
    return False

  if not logFile:
    logFile = "{}/{}".format(logFileFullDirPath, logFileName)
  print("family: logs enabled: setting up logging system.")
